...
EncodeBufr306039::EncodeBufr306039() {}
std::string EncodeBufr306039::logIdentifier() const { return "306039"; }

std::list<int> EncodeBufr306039::encodeIds() const {
  std::list<int> ids;
...
void EncodeBufr306039::encode() {
  ...
  bufr->addValue(22076, v,
                 "DIRECTION FROM WHICH DOMINANT WAVES ARE COMING [DEG]",
                 count(v));

  v = data->WSPRTP.getFirstValueAtLevel(0);
  bufr->addValue(22077, v,
                 "DIRECTION FROM WHICH DOMINANT WAVES ARE COMING [DEG]",
                 count(v));
}

...
#include <kvalobs/fileutil/pidfileutil.h>
#include <kvalobs/kvalobs/kvPath.h>
#include <kvalobs/miutil/getprogname.h>
#include <kvalobs/miutil/timeconvert.h>
#include <kvalobs/kvsubscribe/creategroupid.h>
...
string getValue(const miconf::ConfSection *conf, const std::string &key, bool throwOnMissingKey=true) {
  string val = conf->getValue(key).valAsString("");
  if (val.empty() && throwOnMissingKey) {
...
std::string getKafkaGroupId(const miconf::ConfSection *conf) {
  auto progname=miutil::getProgramName();
  std::string idkey("kafka.groupid."+progname);

  auto val=getValue(conf, idkey, false);
  if ( val.empty() ) {
    idkey="kafka.groupid";
    val=getValue(conf, idkey, false);
  }

  if( val.empty() ){
    idkey="";
    val = kvalobs::subscribe::createKafkaConsumerGroupId();
  }

  if(idkey.empty()) {
    LOGINFO("Autogenerated kafka consumer group id: '" << val << "'")
  } else {
    LOGINFO("Using kafka consumer group id: '" << val << "' from conf key: '" << idkey <<"'");
  }
  return val;
}
...
App::App(int argn, char **argv, Opt &opt, miutil::conf::ConfSection *conf)
  ...
  kafkaGroupId = getKafkaGroupId(conf);
...
void App::run(std::shared_ptr<threadutil::CommandQueue> newDataQue) {
  ...
  if( !options.diasableDataReceiver )
    consumer.reset(new KvDataConsumer(kafkaDomain, kafkaBrokers, kafkaGroupId, newDataQue));

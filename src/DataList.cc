...
DataListEntry::
maxTbTime(long stationid, int typeID)const
{
   boost::posix_time::ptime rt(pt::special_values::min_date_time);
   ...
   for( auto &e : it->second) {
      if( e.stationID() == stationid && e.tbtime() > rt) {
         rt = e.tbtime();
      }
   }
   return rt;

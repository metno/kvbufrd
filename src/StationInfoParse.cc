...
bool
StationInfoParse::DefaultVal::valid() const
{
...
void
StationInfoParse::parseStationDefSections(
  ...
  bool useDefaultValues)
{
  ...
  for (list<string>::iterator it = sect.begin(); it != sect.end(); it++) {
    i = it->find("wmo_");
    ...
    for (ConfSectionList::iterator cit = allSections.begin();
      ...
      try {
        ...
        LOGERROR("FATAL: NOMEM: Cant create internal section <"
                 << NoteSection << ">!" << endl);
      }
      ...
      if (!info) {
        errors << "ERRORS while parsing section <" << *it
               << ">, ignoring the section!" << endl
               << "Reason: " << sErr;
        LOGERROR("Error while parsing section <"
                 << *it << ">, ignoring the section!" << endl
                 << "Reason: " << sErr);
        continue;
      } else if (!sErr.empty()) {
        errors << "WARNINGS while parsing section <" << *it << ">!" << endl
               << "  file: " << sec->getFilename()
               << " line: " << sec->getLineno() << "." << endl
               << "Reason: " << sErr;
        LOGWARN("Warnings while parsing section <"
                << *it << ">." << endl
                << "  file: " << sec->getFilename()
                << " line: " << sec->getLineno() << "." << endl
                << "Reason: " << sErr);
                               ...
                               bool useDefaultValues_)
{
  ...
  //   idTypeValue.size()
  //        << " Value: " <<
  //        (idTypeValue.size()>0?idTypeValue[0].valAsString():"N/A" ) << endl;
  ...
  for (it = keys.begin(); it != keys.end(); it++) {
    ...
    if (!keywords[i]) {
      LOGERROR("Unknown key <" << *it << "> in '" << idType << "' section <"
                               << id << ">!" << endl);
    }
  }
  ...
  for (i = 0; keywords[i]; i++) {
    ...
    if (value.empty() && useDefaultValues) {
      if (strcmp(keywords[i], "precipitation") == 0) {
        ...
        LOGDEBUG("NO VALUE: for key <" << keywords[i] << "> in " << idType
                                       << " section <" << id << ">!. Ignore!");
      } else if (!ignoreMissingValues) {
        curErr << "NO VALUE: Mandatory key: <" << keywords[i]
               << ">! And no default value!" << endl
               << "    Filename: " << stationConf->getFilename()
               << " line: " << stationConf->getLineno() << endl;
      ...
      if (strcmp(keywords[i], "stationid") == 0) {
        ...
        if (value.size() > 1) {
          curErr << "code: can only have one value. The code has #"
                 << value.size() << "." << endl;
          ok = false;
        } else {
        ...
        doInt(st->wmono_, value, st->wmono_);
      } else if (strcmp(keywords[i], "wigos_id") == 0) {
        st->wsiId_ = value.valAsString(st->wsiId_);
      }

      if (!ok) {
        curErr << "    Filename: " << stationConf->getFilename()
               << " line: " << stationConf->getLineno() << endl;
        delete st;
        return 0;
      }
                            ...
                            miutil::conf::ConfSection* stationConf)
{
  ...
  for (it = keys.begin(); it != keys.end(); it++) {
    ...
    if (value.empty()) {
      LOGDEBUG("NOVALUE: for key <" << *it << "> in <" << curSectionName
                                    << "> section! Ignoring it.");
      continue;
    }
...
milog::LogLevel
StationInfoParse::doDefLogLevel(miconf::ValElementList& vl)
{
  string val;
  milog::LogLevel ll;
  ...
  if (ll == milog::NOTSET) {
    ll = milog::INFO;
    LOGERROR("Invalid loglevel value <"
             << val << "> in section <" << curSectionName
             << ">. Setting default loglevel to INFO.");
  }
...
int
StationInfoParse::doDefCode(miconf::ValElementList& vl)
{
  if (vl.size() > 0) {
...
std::string
StationInfoParse::doDefList(miconf::ValElementList& vl)
{
  IValElementList it = vl.begin();
...
std::string
StationInfoParse::doDefOwner(miconf::ValElementList& vl)
{
  IValElementList it = vl.begin();
  ...
  if (val.size() != 4) {
    curErr << "INVALID FORMAT: key <owner> must have a size of four (4) char!"
           << endl;
    return string();
  }
...
std::list<std::string>
StationInfoParse::doDefPrecip(miconf::ValElementList& vl)
{
  const char* validval[] = { "RA",   "RR",    "RRRtr", "RR_01", "RR_1", "RR_3",
                             "RR_6", "RR_12", "RR_24", "RR_N",  0 };
  int i;
  ...
  for (; it != vl.end(); it++, i++) {
    if (it->type() != STRING) {
      curErr << "INVALID TYPE: key <precipitation> element number: " << i << "."
             << endl;
      continue;
    }
    ...
    if (!validval[i]) {
      curErr << "INVALID VALUE: key <precipitation> element number: " << i
             << "." << endl;
      continue;
    }
...
bool
StationInfoParse::doDefCopy(miconf::ValElementList& vl, bool* copyIsSet)
{
  IValElementList it = vl.begin();
  ...
  if (it->type() != STRING && it->type() != ID) {
    curErr << "INVALID TYPE: key <copy>, expecting STRING or ID." << endl;
    cerr << "INVALID TYPE: key <copy>, expecting STRING or ID." << endl;
    return false;
  }
  ...
  if (ch == 't' || ch == 'T')
    ...
    curErr << "WRONG VALUE: key <copy> valid values \"true\" or \"false\"!"
           << endl;
    return false;
  }
}
...
std::string
StationInfoParse::doDefCopyto(miconf::ValElementList& vl)
{
  IValElementList it = vl.begin();
...
StationInfoParse::doDefDelay(const miconf::ValElementList& vl,
                             ...
                             miconf::ConfSection* conf)
{
  ...
  for (int element = 1; it != vl.end(); it++, element++) {
    if (it->type() != STRING) {
      curErr << "INVALID TYPE: key <delay> element number: " << element;
    ...
    if (sign) {
      ...
      try {
        ...
        curErr << "Invalid time spec '" << sign << val << "'. " << ex.what()
               << endl;
        continue;
      }
    ...
    if (sHH.empty() || sMM.empty()) {
      curErr << "INVALID FORMAT: key <delay> element number: " << element
             << ", format \"hh:mm\"!" << endl;
      continue;
    }
    ...
    if (itd == dl.end()) {
      ...
      curErr << "DUPLICATE ELEMENT: key <delay> element number: " << element
             << "." << endl;
      continue;
    } else {
...
bool
StationInfoParse::doStationid(const std::string& key,
                              ...
                              StationInfo& st)
{
  ...
  for (; it != vl.end(); it++, i++) {
    if (it->type() != INT) {
      curErr << "INVALID TYPE: key <" << key << "> element number: " << i
             << endl;
      continue;
    }
    ...
    for (itl = st.definedStationid_.begin(); itl != st.definedStationid_.end();
         itl++) {
...
bool
StationInfoParse::doDelay(const std::string& key,
                          miconf::ValElementList& vl,
                          StationInfo& st,
                          miconf::ConfSection* conf,
                          DefaultValPtr defVal,
...
bool
StationInfoParse::doPrecip(const std::string& key,
                           miconf::ValElementList& vl,
                           StationInfo& st)
{
  ...
  return (vl.empty() && st.precipitation_.empty()) ||
         (!vl.empty() && !st.precipitation_.empty());
}

bool
StationInfoParse::doTypePri(const std::string& key,
                            miconf::ValElementList& vl,
                            StationInfo& st)
{
  bool error = false;
...
void
StationInfoParse::doIntList(std::list<int>& iList,
                            const miconf::ValElementList& val)
{
  int code;
  iList.clear();
...
void
StationInfoParse::doFloat(float& f, const miconf::ValElementList& val)
{
  if (val.empty()) {
    f = FLT_MAX;
    return;

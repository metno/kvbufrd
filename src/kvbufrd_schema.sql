...
CREATE TABLE bufr(wmono integer, id integer, callsign text, code text, obstime timestamp, createtime timestamp, crc integer, ccx integer, data text, bufrBase64 text, tbtime timestamp DEFAULT CURRENT_TIMESTAMP,
             UNIQUE(wmono, id, callsign, code, obstime));

CREATE TABLE waiting( wmono integer, id integer, callsig text, code text, obstime timestamp, delaytime timestamp, UNIQUE(wmono, id, callsign, code, obstime) );

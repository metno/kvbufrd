...
void
getBufrAndOutput(Options &opt, const std::string &query, IntStringMap &nameMap, dnmi::db::Connection *dbCon);
...
int main(int argn, char **argv) {
  ...
  if (options.wmoList.empty()) {
    ...
    for (; it != options.wmoList.end(); it++) {
      ...
      while (obt <= options.ttime) {
        ost.str("");
        ost << "WHERE obstime=\'" << obt.isoTime() << "\' AND wmono=" << wmono;
        getBufrAndOutput(options, ost.str(), nameMap, dbCon);
        obt.addHour(options.tstep);
      }
    }
...
void getBufrAndOutput(Options &opt, const std::string &query, IntStringMap &nameMap, dnmi::db::Connection *dbCon) {
  char buf[30];
  ...
  for (; it != bufrList.end(); it++) {
    ...
    if (opt.onlyBufrtimes && (it->obstime().time_of_day().hours() % 3) != 0)
      continue;

    sprintf(buf, "-%04d%02d%02d%02d.syn", static_cast<int>(it->obstime().date().year()), static_cast<int>(it->obstime().date().month()),
            static_cast<int>(it->obstime().date().day()), static_cast<int>(it->obstime().time_of_day().hours()));
...
bool getDbInfo(miutil::conf::ConfSection* conf, const std::string &useThisConnect) {
...
readConf(const std::string &fname) {
  ...
  if (!fis) {
    ...
    if (!conf) {
      cerr << "Error while reading configuration file: <" << fname << ">!" << endl << parser.getError() << endl;
    } else {
...
void getOptions(int argn, char **argv, Options &opt) {
  struct option long_options[] = { { "help", 0, 0, 0 }, { "only-bufrtimes", 0, 0, 0 }, { 0, 0, 0, 0 } };
...
void use(int exitstatus) {
  cerr << "\n\tuse" << endl << "\t   kvbufrdbclt -f timestamp [-t timestamp] [-s step] " << endl
       << "\t\t[-i stationnames] [-o outdir] [-n wmonolist] [-c dbconnect]" << endl << "\t\t[--only-bufrtimes]" << endl << endl
       << "\t-f timestamp  From time. Use 'all' to ignore fromtime!" << endl << "\t-t timestamp  To time" << endl
       << "\t-s step       Increment the fromtime with this step until totime" << "\n\t          is reached." << endl
       << "\t-o outdir     Write the output to the outdir" << endl << "\t-i stationnames Read the file 'stationsnames' to associate" << endl
       << "\t\twmonumbers with names." << endl << "\t-n wmonolist   a list of wmonumbers to list." << endl
       << "\t-c dbconnect  Use this connect string to connect to the cache database!" << endl << endl << "\t timestamp format: YYYY-MM-DD hh" << endl
       << "\t wmonolist format: n0 n1 n2 .... nN" << endl << endl;

  exit(exitstatus);
}

miutil::miTime toStrToTime(const char *s) {
  int YY, MM, DD, hh;

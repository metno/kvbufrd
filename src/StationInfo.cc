           ...
           int valueUpperLimit)
{
  ...
  if (i == string::npos) {
    ...
    b::split(sVals,
             valToSplit,
             b::is_any_of(string(1, valToSplit[i])),
             b::token_compress_on);
...
MsgTime::MsgTime(bool minutesForEachHour)
  : minAtHour_(0)
{
...
MsgTime::MsgTime(const MsgTime& mt)
  : hours_(mt.hours_)
  , minAtHour_(0)
{
...
bool
MsgTime::operator==(const MsgTime& rhs) const
{
...
bool
MsgTime::msgForTime(const pt::ptime& t) const
{
...
bool
MsgTime::minForEachHour() const
{
  return hours_.test(24);
}

void
MsgTime::reallocateAndSet(int hour, int min)
{
  if (!minForEachHour())
    return;
...
int
MsgTime::getIndexForHour(int h) const
{
  if (!minForEachHour())
    return 0;
...
void
MsgTime::setHour(int h, bool flag)
{
  if (h < 0 || h > 23)
...
void
MsgTime::decodeHourSpec(const std::string& hh_)
{
...
void
MsgTime::decodeMinSpec(const std::string& mm_)
{
  int n;
  int nStart = 0;
  vector<int> iVals;
...
void
MsgTime::setMsgForTime(int hour, int min, bool flag)
{
...
void
MsgTime::setMsgForTime(const std::string& timespec)
{
...
StationInfo::StationInfo(int wmono)
  : code_(-1)
  , height_(INT_MAX)
  , heightVisability_(INT_MAX)
  , heightTemperature_(INT_MAX)
...
StationInfo::StationInfo(const StationInfo& i)
{
...
void
StationInfo::height(int h, bool ifUnset)
{
...
int
StationInfo::heightAdd(int ammount) const
{
  if (height_ != INT_MAX)
...
int
StationInfo::heightVisability() const
{
  return heightVisability_;
}

void
StationInfo::heightVisability(int h, bool ifUnset)
{
...
int
StationInfo::heightTemperature() const
{
  return heightTemperature_;
}

void
StationInfo::heightTemperature(int h, bool ifUnset)
{
...
float
StationInfo::heightPressure() const
{
  return heightPressure_;
}

void
StationInfo::heightPressure(float h, bool ifUnset)
{
...
int
StationInfo::heightPrecip() const
{
  return heightPrecip_;
}

void
StationInfo::heightPrecip(int h, bool ifUnset)
{
...
int
StationInfo::heightWind() const
{
  return heightWind_;
}

void
StationInfo::heightWind(int h, bool ifUnset)
{
...
int
StationInfo::heightWindAboveSea() const
{
  return heightWindAboveSea_;
}

void
StationInfo::heightWindAboveSea(int h, bool ifUnset)
{
...
void
StationInfo::latitude(float lat, bool ifUnset)
{
...
void
StationInfo::longitude(float lon, bool ifUnset)
{
...
bool
StationInfo::wigosIdIsDefined() const
{
  return !wsiId_.empty();
}
...
namespace {
int
intWsiElem(const string& s, const string& elem)
{
...
// throws invalid_argument if wigosid is not defined or there is an error in the
// format.
void
StationInfo::getWigosId(int& identifierSeries,
                        ...
                        std::string& localIdentifier)
{
  ...
  if (wsiElems.size() != 4) {
    ostringstream err;
    err << "Expecting 4 element separeted by -, got " << wsiElems.size() << " '"
        << wsiId_ << "'.";
    throw invalid_argument(err.str());
  }
...
int
StationInfo::callsignAsInt() const
{
  try {
...
void
StationInfo::name(const std::string& n, bool ifUnset)
{
...
int
StationInfo::stationID() const
{
  return stationID_;
}

bool
StationInfo::hasDefinedStationId(long stid) const
{
...
bool
StationInfo::hasDefinedStationIdAndTypeId(long stid, long tid, int hour) const
{
  return hasDefinedStationId(stid) && hasTypeId(tid, hour);
}

StationInfo::TLongList
StationInfo::typepriority(int hour) const
{
  TLongList ret;
...
std::list<int>
StationInfo::continuesTypeids(const std::list<int>& ctList) const
{
...
StationInfo::TLongList
StationInfo::mustHaveTypes(int hour) const
{
  TLongList ret;
  CITTypeList it = typepriority_.begin();
...
bool
StationInfo::hasTypeId(int typeID, int hour) const
{
  CITTypeList it = typepriority_.begin();
...
bool
StationInfo::mustHaveType(int typeid_, int hour) const
{
  TLongList ret;
  CITTypeList it = typepriority_.begin();
...
bool
StationInfo::msgForTime(const pt::ptime& t) const
{
  if (t.is_special())
    return false;
  ...
  for (CITDelayList it = delayList_.begin(); it != delayList_.end(); ++it)
    if (it->skipMsgSpec())
...
std::string
StationInfo::codeToString() const
{
  if (code_ < 0)
    return "0";
...
int
StationInfo::delay(const pt::ptime& t, bool& force, bool& relativToFirst) const
{
  if (t.is_special())
    return 0;
...
std::string
StationInfo::toIdentString() const
{
  ostringstream o;
...
bool
StationInfo::operator<(const StationInfo& rhs) const
{

  if ((wsiId_ < rhs.wsiId_) || (wsiId_ == rhs.wsiId_ && wmono_ < rhs.wmono_) ||
      (wsiId_ == rhs.wsiId_ && wmono_ == rhs.wmono_ &&
       stationID_ < rhs.stationID_) ||
      (wsiId_ == rhs.wsiId_ && wmono_ == rhs.wmono_ &&
       stationID_ == rhs.stationID_ && callsign_ < rhs.callsign_) ||
      (wsiId_ == rhs.wsiId_ && wmono_ == rhs.wmono_ &&
       stationID_ == rhs.stationID_ && callsign_ == rhs.callsign_ &&
       stationID_ < rhs.stationID_)) {
    return true;
...
StationInfo::operator==(const StationInfo& rhs) const
{
  if (wmono_ == rhs.wmono_ && stationID_ == rhs.stationID_ &&
      callsign_ == rhs.callsign_ && wsiId_ == rhs.wsiId_ &&
      code() == rhs.code()) {
    return true;
  } else {
    return false;
...
std::ostream&
StationInfo::printDelayInfo(std::ostream& ost)
{
  for (CITDelayList it = delayList_.begin(); it != delayList_.end(); ++it)
...
std::string
StationInfo::getStationsAndTypes() const
{
  ostringstream ost;
  bool first = true;
...
std::ostream&
operator<<(std::ostream& ost, const StationInfo& sd)
{
  ost << "                name: " << sd.name() << endl;
  ost << "                code: " << sd.code() << endl;
  ost << "            idstring: " << sd.toIdentString() << endl;
  ost << " wmo/stationID/callsign/wsiID: " << sd.wmono() << "/"
      << sd.stationID() << "/" << sd.callsign() << "/" << sd.wsiId_ << endl;
  ...
  ost << "           delayLogic: "
      << (!sd.delayList_.empty() ? "TRUE" : "FALSE") << endl;
...
std::ostream&
operator<<(std::ostream& ost, const DelayInfo& sd)
{
  if (sd.undef()) {
    ost << "\"*** UNDEFINED ***\"";
    return ost;
...
StationInfoList
StationList::findStation(long sid,
                         long tid,
                         const boost::posix_time::ptime& obstime) const
{
  ...
  if (tid == 0)
    for_each(begin(), end(), [=, &r](StationInfoPtr s) {
      if (s->hasDefinedStationId(sid))
        r.push_back(s);
    });
  else
    for_each(begin(), end(), [=, &r](StationInfoPtr s) {
      if (s->hasDefinedStationIdAndTypeId(sid, tid, h))
        r.push_back(s);
    });
...
StationInfoList
StationList::findStationByWmono(int wmono) const
{
  StationInfoList ret;

  if (wmono > 0) {
    for (StationList::const_iterator it = begin(); it != end(); ++it) {
      if ((*it)->wmono() == wmono) {
        ret.push_back(*it);
      }
    }
...
StationInfoList
StationList::findStationById(int id) const
{
...
StationInfoList
StationList::findStationByCallsign(const std::string& callsign) const
{
  StationInfoList ret;
...
StationInfoList
StationList::findStationByWsiId(const std::string& wsiId) const
{
  StationInfoList ret;
...
StationInfoCompare::StationInfoCompare(const StationList& removedStations,
...
StationInfoCompare::StationInfoCompare() {}

StationInfoCompare::StationInfoCompare(const StationInfoCompare& s)
...
StationInfoPtr
StationInfoCompare::findStation(const StationList& stationList,
                                StationInfoPtr station)
{
  for (StationList::const_iterator it = stationList.begin();
       it != stationList.end();
       ++it) {
    if (**it == *station) {
      return *it;
...
StationInfoCompare
StationInfoCompare::compare(const StationList& oldConf,
                            const StationList& newConf)
{
  ...
  for (StationList::const_iterator it = oldConf.begin(); it != oldConf.end();
       ++it) {
  ...
  for (StationList::const_iterator it = newConf.begin(); it != newConf.end();
       ++it) {
